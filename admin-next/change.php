        ...
        <div style="padding: 15px;">
            ...
            <script>
                layui.use(['table', 'dropdown', 'form', 'layer'], function () {
                    ...
                    table.render({
                        elem: '#icp_table',
                        url: '/api/v1/geticpchange.php', // 此处为静态模拟数据，实际使用时需换成真实接口
                        toolbar: '#toolbar',
                                ...
                                click: function (menudata) {
                                    if (menudata.id === 'pass') {
                                        layer.confirm('请确定是否通过 - id:' + data.id + ' - 网站名：' + data.website_name + ' - 备案号：' + data.icp_number + ' 的备案信息？', function (index) {
                                            ...
                                            $.ajax({
                                                type: 'POST',
                                                url: '/api/v1/recordchange.php', // 后端处理脚本的 URL
                                        ...
                                        layer.confirm('确认是否联系 - id:' + data.id + ' - 网站名：' + data.website_name + ' - 备案号：' + data.icp_number + '的所有者？', function (index) {
                                            layer.msg('已提交，服务器处理中');
                                            layer.close(index);
                                            // 向服务端发送指令
                                            var customData = {
