...
if (!empty($owner) || !empty($site_name) || !empty($qq) || !empty($icp_number) || !empty($site_url)) {
    // 执行搜索函数
    search($owner, $site_name, $qq, $icp_number, $site_url);
} else {
    // 执行分页函数
    get($page, $limit);
}

function search($owner, $site_name, $qq, $icp_number, $site_url): void
{
    global $pdo;
    ...
    if (!empty($site_name)) {
        $conditions[] = "website_name = :site_name";
    ...
    if (!empty($icp_number)) {
        $conditions[] = "icp_number = :icp_number";
...
// 查询总记录数
    $totalQuery = "SELECT COUNT(*) FROM icp_records_change";
...
// 查询分页数据
    function fetchPaginatedData($pdo, $limit, $offset)
    {
       ...
       icp_number as 'icp_number', owner as 'owner', update_time as 'update_time',
       STATUS as 'STATUS', email as 'email', qq as 'qq'
FROM icp_records_change ORDER BY id LIMIT :limit OFFSET :offset";
